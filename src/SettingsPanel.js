...
export default function SettingsPanel({ settings, updateSettings, pageLang, downloadImage, wrapLines }) {
  const loc = i18n[pageLang].loc.settings;

  function autoWrap(e) {
    const wrapped = wrapLines(settings.dialogueText, settings.dialogueType, pageLang);
